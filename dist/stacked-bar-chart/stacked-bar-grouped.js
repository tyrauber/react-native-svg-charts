...
var stacked_bar_grouped_exports = {};
__export(stacked_bar_grouped_exports, {
  default: () => stacked_bar_grouped_default
});
module.exports = __toCommonJS(stacked_bar_grouped_exports);
...
// src/stacked-bar-chart/stacked-bar-grouped.tsx
var StackedBarGrouped = class extends import_react2.PureComponent {
  constructor() {
    super(...arguments);
    this.state = {
      width: 0,
  ...
  calcAreas(x, y, series) {
    ...
    if (horizontal) {
      ...
      areas = series.map((stack2, stackIndex) => {
        return stack2.map((serie, keyIndex) => {
      ...
      areas = series.map((stack2, stackIndex) => {
        return stack2.map((serie, keyIndex) => {
          return serie.map((entry, entryIndex) => {
            ...
            const showTopBorder = keyIndex === stack2.length - 1;
  ...
  getSeries() {
    ...
    return data.map(
      (obj, index) => shape.stack().keys(keys[index]).value((item, key) => valueAccessor({ item, key })).order(order).offset(offset)(obj.data)
    );
  }
  render() {
