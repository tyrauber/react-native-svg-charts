...
var AreaChart = ({ start = 0, ...props }) => {
  const createPaths = ({ data, x, y }) => {
    const { curve } = props;
    const area2 = shape2.area().x((d) => x(d.x)).y0(y(start)).y1((d) => y(d.y)).defined((item) => typeof item.y === "number").curve(curve)(data);
    const line2 = shape2.line().x((d) => x(d.x)).y((d) => y(d.y)).defined((item) => typeof item.y === "number").curve(curve)(data);
    return {
      path: area2,
      area: area2,
      line: line2
